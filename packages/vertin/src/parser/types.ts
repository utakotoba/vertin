/**
 * @name ResolvableParameter
 * @description Constructor types that the parser can resolve into runtime values.
 ...
 */
export type ResolvableParameter
 ...
 * @private
 * @description Utility type to map a {@link ResolvableParameter}
 * into its corresponding TypeScript primitive type.
 *
 ...
 * - `BooleanConstructor[]` → `boolean[]`
 */
export type _ResolveParameterType<T extends ResolvableParameter>
 ...
 */
export type _ParserBaseOption<T extends ResolvableParameter> = {
  /**
...
export type ParserFlagOption
  = ResolvableParameter extends infer T ? (
    T extends ResolvableParameter ? _ParserBaseOption<T> & {
      /**
